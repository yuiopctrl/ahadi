...
begin
  ...
  execute format($ddl$
    ...
    begin
      ...
      key := coalesce(nullif(p_idempotency_key, ''), md5(coalesce(p_plan_code, '') || ':' || coalesce(p_tenant_name, '') || ':' || coalesce(p_first_event_name, '')));
      request_hash := encode(%1$I.digest(coalesce(p_plan_code, '') || ':' || coalesce(p_tenant_name, '') || ':' || coalesce(p_tenant_phone, '') || ':' || coalesce(p_first_event_name, ''), 'sha256'), 'hex');
      ...
      if profile_record.onboarding_completed_at is not null and exists (
        select 1
...
revoke all on function public.rpc_complete_tenant_onboarding(text, text, text, text, text, text, date, text, numeric, date, text) from public;
grant execute on function public.rpc_complete_tenant_onboarding(text, text, text, text, text, text, date, text, numeric, date, text) to authenticated;
