...
test('tenant layout navigation follows the route event instead of the tenant default', () => {
  ...
  assert.match(tenantHeader, /function EventNavigation/)
  assert.match(tenantHeader, /<NavLink to=\{eventBase\} end>Dashboard<\/NavLink>/)
...
test('mobile app chrome stays minimal and avoids global record-payment actions', () => {
  ...
  assert.match(styles, /@media \(min-width: 960px\)[\s\S]+\.unified-header-event-nav \{\s+display: flex;/)
