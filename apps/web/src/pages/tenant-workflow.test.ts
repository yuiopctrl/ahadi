...
const tenantHeader = readFileSync(new URL('../components/tenant-header.tsx', import.meta.url), 'utf8')
...
test('tenant layout navigation follows the route event instead of the tenant default', () => {
  ...
  assert.match(tenantLayout, /<DesktopSidebar event=\{event\}/)
  assert.match(tenantLayout, /<MobileBottomNav event=\{event\}/)
  assert.match(tenantLayout, /onEventChange=\{handleEventChange\}/)
  assert.match(tenantLayout, /<UnifiedTenantHeader/)
  assert.doesNotMatch(tenantLayout, /<EventSnapshotBar/)
  assert.doesNotMatch(tenantLayout, /<TenantTopNav/)
  ...
  assert.doesNotMatch(navigation, /export function MobileTopBar/)
  assert.doesNotMatch(navigation, /export function TenantTopNav/)
  assert.doesNotMatch(navigation, /<EventContextDisplay event=\{event\} \/>/)
  assert.doesNotMatch(tenantHeader, /function EventNavigation/)
  assert.doesNotMatch(tenantHeader, /NavLink/)
  assert.doesNotMatch(tenantHeader, />Dashboard</)
  assert.doesNotMatch(tenantHeader, />Members</)
  assert.doesNotMatch(tenantHeader, />Reports</)
  assert.match(tenantHeader, /function resolveDisplayName\(context: UserContext \| null, user: SupabaseUser \| null\)/)
  assert.match(tenantHeader, /context\?\.profile\?\.fullName/)
  assert.match(tenantHeader, /metadataName\(user, 'display_name'\)/)
  assert.match(tenantHeader, /context\?\.profile\?\.phoneE164/)
  assert.match(tenantHeader, /context\?\.profile\?\.email/)
  assert.match(tenantHeader, /\|\| 'User'/)
  assert.doesNotMatch(tenantHeader, /Ahadi user/)
  assert.match(tenantHeader, /<span className="account-avatar">\{initialsFor\(name\)\}<\/span>/)
  ...
  assert.match(sessionStore, /clearStoredEventSelections/)
  assert.match(sessionStore, /clearTenant\(true\)/)
...
test('mobile app chrome stays minimal and avoids global record-payment actions', () => {
  ...
  assert.match(styles, /\.unified-tenant-header \{/)
  assert.match(styles, /\.unified-header-mobile-details/)
  assert.doesNotMatch(styles, /\.unified-header-event-nav/)
  assert.doesNotMatch(styles, /\.unified-header-event-menu/)
...
test('organization switcher exposes active tenant switching and creation from inside the app', () => {
  assert.match(tenantHeader, /TenantSwitcherDisplay tenant=\{tenant\}/)
  assert.match(tenantLayout, /<UnifiedTenantHeader/)
