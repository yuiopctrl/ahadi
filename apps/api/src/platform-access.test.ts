...
test('platform middleware authorizes from platform permissions and returns 403 for inactive or missing permission', () => {
  ...
  assert.match(middleware, /function hasPlatformPermission/)
  assert.match(middleware, /context\.platformRole === 'PLATFORM_OWNER' && permission\.startsWith\('platform\.'\)/)
  ...
  assert.match(middleware, /hasPlatformPermission\(context, permission\)/)
